...
environmental_factor <- simulate_env_flucs(5, 365 * 1,1)

plot(environmental_factor,type = 'l')


growth_rate_df<- simulate_variablity_species(sigma_i = 10, r0 = 0.2, 
                                             E_0 = 0, r_sigma_0 = 20, 10)|>
  apply(1, function(x) simulate_gaussian_curve(x,E = seq(-100,100)))|>
  data.frame()

growth_rate_df$E <-  seq(-100,100)
...
beta_matrix <- simulate_betas(n_species = 10, mean_beta = 1e-3,inter_mult = 0.20, CV_desired =  0.01)
hist(beta_matrix)
