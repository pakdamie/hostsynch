...
calculate_establishment_proportion <- function(
  ...
  for (i in seq(1, length(breadth_rep_list))) {
    ...
    RE_time <- lapply(model_sim, function(x) {
      calculate_R_effective(x,
        time = timestep,
        b_matrix = bmatrix,
        params =
          create_parameters(timestep, "standard")
      )[500:(timestep), ]
    })
    ...
    RE_greater_1 <- lapply(RE_time, function(x) {
      ...
      cbind(
        nrow(valid_rows) / (timestep - 500), # Compute proportion
    ...
    summary_list[[i]] <- cbind.data.frame(
      synch_value = do.call(rbind, synchrony_value),
...
calculate_establishment <- function(n_species, 
                                    timestep,
                                    bmatrix_list,
                                    rmatrix_list) {
  ...
  RE_time <- lapply(seq_along(model_sim), function(i) {
  calculate_R_effective(model_sim[[i]],
    time = timestep,
    b_matrix = bmatrix_list[[i]], 
    params = create_parameters(timestep, "standard")
  )[500:timestep, ]
})
...
ggplot(tmp2, 
  aes(x = synch_value, y = prop_invadable)) +
  geom_point(size = 1.2) + 
  xlab("Community Synchrony") + 
  ylab("Proportion of time R0 > 1") +
  theme_classic() + 
  theme(axis.text = element_text(size = 13),
        axis.title = element_text(size = 14)) + 

ggplot(tmp2, aes(x = synch_value, y = mean_RE)) +
  geom_point(size = 1.2) + 
  xlab("Community Synchrony") + 
  ylab("Average R0") +
  theme_classic() + 
  ...
  ggplot(tmp2, aes(x = synch_value, y = CV_RE)) +
  geom_point(size = 1.2) + 
  xlab("Community Synchrony") + 
  ylab("R0 CV") +
  theme_classic() + 
...
ggsave("establishment_plots_varybmat.pdf", units = "in",
       width = 20, height= 6)
